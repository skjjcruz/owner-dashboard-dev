 ...
 * Sleeper Compare + Stats + Picks (Year blocks)
 *
 * Columns:
 ...
 *
 * USER LOCK FEATURE:
 * - First username entered is locked on device (localStorage)
 * - User can see ALL their leagues
 * - Cannot switch to other users inside app
 *************************************************/

const SPORT = "nfl";
const DEFAULT_USERNAME = "skjjcruz"; // only used if NO locked user exists
...
// ===== USER LOCK =====
const LOCK_KEY = "ownerDashLockedUsername"; // do not change once released
const lockedUsername = localStorage.getItem(LOCK_KEY);
...
// Remove any “change user” button if it exists (old experiments)
(function removeChangeUserUI() {
  const byId = document.getElementById("changeUserBtn");
  if (byId) byId.remove();

  document.querySelectorAll(".changeUserBtn").forEach((n) => n.remove());
})();

// Set initial form values
elSeason.value = DEFAULT_SEASON;

// If locked, enforce it and disable input.
// If not locked, allow typing (first run).
if (lockedUsername) {
  elUsername.value = lockedUsername;
...
// App state
let state = {
  username: lockedUsername || DEFAULT_USERNAME,
...
async function loadLeagues() {
  // If locked, force the locked username no matter what
  const typed = (elUsername.value || "").trim();
  state.username = lockedUsername || typed;
  ...
  // On first successful login, LOCK IT
  if (!lockedUsername) {
    localStorage.setItem(LOCK_KEY, state.username);
    elUsername.value = state.username;
    elUsername.disabled = true;
  }
