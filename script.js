 ...
 *
 * Columns:
 * - Yrs (from players dictionary years_exp)
 * - Pts / GP from Sleeper season stats
 * - Avg = Pts / GP
 * - Draft picks (2026–2028) grouped by year at bottom
 *
 * v1.1 change:
 * - Any owner can use it: username saved once (localStorage) + Change User button
 *************************************************/

const SPORT = "nfl";
const DEFAULT_SEASON = "2025"; // you can keep this fixed, or set to new Date().getFullYear().toString()

const POS_ORDER = ["QB", "RB", "WR", "TE", "K", "DEF", "DL", "LB", "DB", "OTHER"];
const PICK_YEARS = [2026, 2027, 2028];

/* ===== LocalStorage keys (user lock) ===== */
const LS_USERNAME = "ownerdash_username";
const LS_SEASON = "ownerdash_season";
...
/* =========================================================
   USER LOCK (one-time login) – v1.1
   ========================================================= */
function injectChangeUserButton() {
  if (document.getElementById("changeUserBtn")) return;

  const btn = document.createElement("button");
  btn.id = "changeUserBtn";
  btn.textContent = "Change User";

  // keep it visually consistent without forcing CSS changes
  btn.style.height = "34px";
  btn.style.padding = "0 12px";
  btn.style.borderRadius = "10px";
  btn.style.border = "1px solid #d7dbe3";
  btn.style.background = "#f7f8fb";
  ...
  btn.addEventListener("click", () => {
    ...
    localStorage.removeItem(LS_SEASON);
    location.reload();
  });

  elReloadBtn.insertAdjacentElement("afterend", btn);
}
...
function loadStoredCredentials() {
  ...
  if (savedUser) {
    ...
    injectChangeUserButton();
  }

  if (savedSeason) {
    state.season = savedSeason;
    elSeason.value = savedSeason;
  } else {
    elSeason.value = state.season;
  }
}

function saveCredentials(username, season) {
...
async function loadLeagues() {
  ...
  state.season = (elSeason.value || "").trim();
  ...
  // Save credentials AFTER successful user lookup
  saveCredentials(state.username, state.season);
...
async function loadLeagueData() {
  state.leagueId = elLeagueSelect.value || state.leagueId;
  ...
  setStatus("Ready ✅ (Tap L / R on a team)");
}
...
(function boot() {
  ...
  // If we already have a saved username, auto-run
  if (elUsername.value && elUsername.disabled) {
    fullReload();
  } else {
    // Otherwise, just set defaults in the boxes and wait for Reload
    if (!elSeason.value) elSeason.value = DEFAULT_SEASON;
    setStatus("Enter username, then hit Reload.");
  }
})();
